...
public class ServerDownloadHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void messageReceived(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        final String path = resolveUri(uri);

        checkFile(ctx,path,uri);
     ...
     * @date 21/3/24 15:58
     */
    private void checkFile(ChannelHandlerContext ctx,String path,String uri){
        if (path == null) {
     ...
     * @date 21/3/24 15:58
     */
    private void checkRequest(ChannelHandlerContext ctx,FullHttpRequest request){
        // 1、对请求的解码结果进行判断
